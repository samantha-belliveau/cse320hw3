...
Test(bud_free_suite, free_pointer_twice, .init = bud_mem_init, .fini = bud_mem_fini,
     .timeout = 5, .signal = SIGABRT) {
    errno = 0;
    int *x = bud_malloc(sizeof(int));
    ...
    bud_free(x);    // free twice

}

Test(bud_free_suite, invalid__pointer_alignment, .init = bud_mem_init, .fini = bud_mem_fini,
     .timeout = 5, .signal = SIGABRT) {
    errno = 0;
    void *x = bud_malloc(sizeof(int));

    x = x + 3; // no longer 8 byte alligned
    bud_free(x);

}


Test(bud_malloc_suite, request_invalid_size, .init = bud_mem_init, .fini = bud_mem_fini,
     .timeout = 5) {
    errno = 0;

    int *y = bud_malloc(0); // invalid zero
    cr_assert_null(y);
    expect_errno_value(EINVAL);

    errno = 0;

    y = bud_malloc(16384); // too large
    cr_assert_null(y);
    expect_errno_value(EINVAL);

    errno = 0;

    y = bud_malloc(16376); // largest valid size
    cr_assert_not_null(y);
    expect_errno_value(0);
}

Test(bud_realloc_suite, realloc_same_order_block, .init = bud_mem_init, .fini = bud_mem_fini,
     .timeout = 5) {

    errno = 0;

    void *ptr = bud_malloc(16);
    void *newPtr = bud_realloc(ptr, 17);

    cr_assert(ptr == newPtr);

    bud_header *header = newPtr - sizeof(bud_header);
    assert_header_values(header, ALLOCATED, 5, PADDED, 17); // new block should be same address with
                                                            // rsize = 17

    void *ptr1 = bud_malloc(16);
    void *newPtr1 = bud_realloc(ptr1, 24);

    cr_assert(ptr1 == newPtr1);

    bud_header *header1 = newPtr1 - sizeof(bud_header);

    assert_header_values(header1, ALLOCATED, 5, UNPADDED, 24);  // new block should be same address with
                                                                // rsize = 24 and unpadded
    expect_errno_value(0);
}
...
Test(bud_realloc_suite, realloc_NULL_pointer, .init = bud_mem_init, .fini = bud_mem_fini,
    ...
    x = bud_realloc(x, 64); // realloc NULL pointer ==> malloc pointer of size 64

    cr_assert_not_null(x);

    bud_header *header = x - sizeof(bud_header);
    assert_header_values(header, ALLOCATED, 7, PADDED, 64);
